...
class EmployeeList1(AsyncWebsocketConsumer):
    ...
    def get_employee(self, employee_mail):
        return User.objects.get(email=employee_mail)

    @database_sync_to_async
    def get_other_employee(self, org_id, excluded_emp_id):
        return list(TMEmployeeDetail.objects.filter(Org_id=org_id).exclude(id=excluded_emp_id))
        ...
        for emp in employees:
            ...
            try:
                chat = EmployeeChat.objects.filter(
                    Q(sender__id=emp.employee_id, receiver__id=current_employee_id) |
                    Q(sender__id=current_employee_id, receiver__id=emp.employee_id)
    ...
    def get_unread_notifications_count(self, employee_id):
        ...
        for chat in EmployeeChat.objects.filter(
            Q(receiver__id=employee_id) | Q(messages__icontains=employee_id)
        ):
